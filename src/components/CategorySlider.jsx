...
const CategorySlider = ({ selectedCategory, setSelectedCategory }) => {
    return (
        <div className="flex gap-5">
            <div>
                <Button className={`mt-[1.4rem] p-0 h-[5rem] bg-white ${selectedCategory === "All" && "border-2 border-blue-700"}`} onClick={() => setSelectedCategory("All")}>
            ...
            <div className="w-full overflow-x-auto no-scrollbar">
                <div className="grid grid-flow-col grid-cols-3 gap-5 w-max">
                    {DataKategori.map(category => {
                        return (
                            <Button className={`mt-[1.4rem] p-0 h-[5rem] bg-white ${selectedCategory === category.name && "border-2 border-blue-700"}`} key={category}  onClick={() => setSelectedCategory(category.name)}  disabled={category.name === "segera hadir"}>
